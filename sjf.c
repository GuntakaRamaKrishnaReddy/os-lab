...
void main()
	{
		...
		for(i=1;i<=n;i++)
		  {
		  	printf("/n enter process %d bursttime:",i);
		  	scanf("%d",&burst[i]);
		  }
		for(i=1;i<=n;i++)
		  {
		...
		printf("/n burst start finish wait/n");
		for(i=1;i<=n;i++)
		  {
		...
		avgwait=totalwait/n;
		avgturn=totalturn/n;
		printf("average waiting time %f/n",avgwait);
		printf("average turnaround time %f/n",avgturn);
		//getch();
	}
